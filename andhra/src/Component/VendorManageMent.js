...
export default function Vendor() {
  ...
  const getAllVendorInfo = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        let filterCityWise = vendor?.filter((ele) => ele.vendorState === "Andhrapradesh");
  ...
  const AddVendorBankData = async () => {
    ...
    formData.append("vendorStatus", true);
                ...
                {vendorData ? (
                  vendorData?.map((ele, index) => {
                    ...
                    if (Array.isArray(recceData)) {
                      recceData?.forEach((recce) => {
                        recce.outletName?.forEach((vend) => {
                          if (vend.vendor === ele._id) {
                            AssignedJob++;
                          }
                        });
                      });
                    }
                    let completedid = OutletDoneData?.map(
                      (outlet) => outlet.outletShopId
                    );
                      ...
                      >
                        <div className="row  m-auto">
                          ...
                          <span style={{ color: "orange" }}>
                            Job Assigned
                            <span style={{ margin: "5px" }}>{AssignedJob}</span>
                          </span>
                        ...
                        <div className="col-md-12">
                          ...
                          >
                            ...
                            <p>
                              ...
                              <span>{vendorData[selected].PassWord}</span>
                            </p>
