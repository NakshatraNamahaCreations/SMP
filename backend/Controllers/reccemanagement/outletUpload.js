...
class outLetBoardManagement {
  ...
  async updateOutletBoard(req, res) {
    try {
      const id = req.params.id;
      const { category, height, width, unitsOfMeasurment, quantity, remark,angles } =
        req.body;
      const file = req.file?.filename;
      const findJob = await outletBoardModal.findOne({ _id: id });
      if (!findJob) {
      ...
      if (file) findJob.ouletBannerImage = file;
      if (remark) findJob.remark = remark;
      if (angles) findJob.angles = angles; //01-02-2024
  ...
  async completeJob(req, res) {
    try {
      ...
      const missingImageShop = await outletBoardModal.findOne({
        _id: { $in: shopIds },
        ouletBannerImage: { $exists: false },
      });

      if (missingImageShop) {
        return res.status(400).json({
          error: "Please upload an image and complete the job.",
        });
      } else {
        return res.status(200).send({
          statusCode: 200,
