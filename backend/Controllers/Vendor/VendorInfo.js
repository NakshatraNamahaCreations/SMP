  ...
  async VendorLoginWitheEmail(req, res) {
    ...
    try {
      if (!VendorEmail || !PassWord) {
        ...
        const user = await VendorInfoModel.findOne({ VendorEmail, PassWord });

        if (!user) {
          return res.status(400).json({ error: "Invalid credentials" });
        }

        if (user.vendorStatus === "Online") {
          return res.status(400).json({ error: "User is already logged in" });
        }
        await VendorInfoModel.updateOne(
          { _id: user._id },
          { vendorStatus: "Online" }
        );
        const responseUser = {
        ...
        return res
          .status(200)
          .json({ success: "Login Successfully", user: responseUser });
      }
    } catch (err) {
